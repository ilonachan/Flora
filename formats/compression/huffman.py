...
class BitInt:
    ...
    def concat(self, other: "BitInt") -> "BitInt":
        return BitInt((self.val & self.mask) << other.len | (other.val & other.mask), self.len + other.len)
...
class HuffmanNode(Generic[T]):
    ...
    def to_mapping(self, prefix: BitInt = BitInt(0, 0)) -> Mapping[BitInt, T]:
        ...
        map = {}
        if self.child0 is not None:
            map |= self.child0.to_mapping(self, prefix.concat(BitInt.ZERO))
        if self.child1 is not None:
            map |= self.child1.to_mapping(self, prefix.concat(BitInt.ONE))
        return map
    ...
    def get_at_stream(self, key: BitInt, fetch_more: Callable[[], BitInt]) -> Tuple[T, BitInt]:
        if self.data is not None:
...
def node_from_bytes(stream: ByteReader, end: int, is_data: bool, parent: Optional[HuffmanNode[int]] = None) -> Optional[HuffmanNode[int]]:
    start = stream.loc
...
def decompress(data):  # sourcery skip: remove-unreachable-code
    ...
    root = node_from_bytes(data, treeEnd)
    ...
    while outPos < dataLen:
        def read_next_window():
            ...
            return val
        data, window = root.get_at_stream(window, read_next_window)
